 ...
 *
 */
public class Red_LoadSideGearOnly extends CommandGroup {

	double driveSpeed;
	double driveSpeedOrient;
	public Red_LoadSideGearOnly() {
		// Add Commands here:
		...
		double dist2Turn = 71;//68.5;
		double targetPoint = 60.5;//58.7;
		double turnAngle = 60;
		...
		driveSpeedOrient = Robot.prefs.getDouble("DriveSpeedOrient", .5);
...
//		addSequential(new TimeDelay(.1));
		...
		addSequential(new RobotPosition(200, driveSpeed, MoveMode.absolute, CompMode.gyro, false, .75, true, 6));
